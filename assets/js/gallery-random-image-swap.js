document.addEventListener('DOMContentLoaded', function () {
  // Pool of all available images (relative paths — baseurl is added when set as src)
  const imagePool = [
  ...
  // Mapping of image paths to alt text descriptions (keyed by the same relative paths)
  const imageAltText = {
  ...
  // siteBaseUrl is set globally in the page's <script> block from {{ site.baseurl }}
  // Falls back to an empty string if it hasn't been defined, so this still works standalone.
  const baseUrl = typeof siteBaseUrl !== 'undefined' ? siteBaseUrl : '';

  // Build the full src for a relative image path
  function withBaseUrl(path) {
    return baseUrl + path;
  }

  // Strip the baseurl prefix back off a resolved pathname so it matches
  // the relative paths used as keys in imagePool / imageAltText / currentImages
  ...
  function initializeGallery() {
    ...
    galleryItems.forEach(function (img, index) {
      ...
      img.src = withBaseUrl(randomImage);
  ...
  function swapSingleImage(imgElement) {
    ...
    // then strip the baseurl so it matches the keys used in imagePool/imageAltText
    const currentSrc = stripBaseUrl(new URL(imgElement.src).pathname);
    currentImages.delete(currentSrc);
    ...
    setTimeout(() => {
      imgElement.src = withBaseUrl(newImage);
